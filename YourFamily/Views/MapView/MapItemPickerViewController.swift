    ...
    class MapItemPickerViewController:
        ...
        lazy var searchNavigationController: UINavigationController = {
            let searchNavigationController = UINavigationController(
                rootViewController: searchResponseTableViewController)
        ...
        private lazy var searchCompletionsTableViewController:
            SearchCompletionsTableViewController = {
                ...
                tableViewController.tableView.separatorEffect = UIVibrancyEffect(
        ...
        private lazy var searchResponseTableViewController: SearchResponseTableViewController = {
            ...
            tableViewController.onMapItemSelection = { mapItemIndex in
                ...
                if !self.mapView.annotations(in: self.mapView.visibleMapRect).contains(
                    annotation as! AnyHashable)
                {
            ...
            tableViewController.onViewWillLayoutSubviews = {
                ...
                self.mapView.layoutMargins = UIEdgeInsets(
                    top: 0, left: 0, bottom: bottomMargin, right: 0)
        ...
        private lazy var selectionButton: UIBarButtonItem = {
            let selectionAction = UIAction { _ in
                if let index = self.selectedAnnotationIndex,
                    let response = self.searchResponse?.mapItems[index] {
                    self.onDismiss?(response)
                }
            }
            let button = UIBarButtonItem(
                systemItem: .done, primaryAction: selectionAction, menu: nil)
        ...
        private lazy var searchController: UISearchController = {
            let searchController = UISearchController(
                searchResultsController: searchCompletionsTableViewController)
        ...
        override func viewDidAppear(_ animated: Bool) {
            ...
            foregroundRestorationObserver = NotificationCenter.default.addObserver(
        ...
        func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
            ...
            if let annotationIndex = annotationIndex {
                searchResponseTableViewController.tableView.selectRow(
                    at: IndexPath(row: annotationIndex, section: 0), animated: true,
        ...
        func mapView(_ mapView: MKMapView, didDeselect view: MKAnnotationView) {
            ...
            if let annotationIndex = annotationIndex {
                searchResponseTableViewController.tableView.deselectRow(
                    at: IndexPath(row: annotationIndex, section: 0), animated: true)
            }
        }
            ...
            didSet {
                if let coordinate = initalUserLocation?.coordinate {
                    let region = MKCoordinateRegion(
                        center: coordinate, latitudinalMeters: 12_000, longitudinalMeters: 12_000)
